...
// Perform a partial string search
exports.filter = async (req, res) => {
  try {
    ...
    for (const column of columns) {
      const condition = {};
      condition[column] = { [Op.regexp]: `(${filterText})` };
      criteria[Op.or].push(condition);
    }
...
exports.deleteMultiple = async (req, res) => {
  const ids = req.body.ids; //Array of IDs to delete
  ...
  try {
    ...
    res.send({
      message: `${num} Authentication were deleted successfully!`,
    });
  } catch (err) {
    res.status(500).send({
      message: "Could not delete Authentication.",
      error: err.message
    });
  }
};
